...
public class MapsActivity extends FragmentActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        DBManager dbManager = new DBManager(this, 1);
        ...
        if (from.equals("search")) {
            place = dbManager.getPlaceByID(id);

            Location current = getIntent().getParcelableExtra("current");
            if (current != null) {
                LatLng cLatLan = new LatLng(current.getLatitude(), current.getLongitude());
    ...
    private void setUpMap() {
       // mMap.addMarker(new MarkerOptions().position(new LatLng(32.0878802,34.797246)).title("Marker"));
